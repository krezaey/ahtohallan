...
const xinc = new ast.Increment(x, "++")
const xdec = new ast.Increment(x, "--")
...
const tests = [
  ...
  ["folds negation", new ast.UnaryExpression(0, "-", 8), -8],
  ...
  ["removes x=x at beginning", [new ast.Assignment(x, x), xinc], [xinc]],
  ["removes x=x at end", [xinc, new ast.Assignment(x, x)], [xinc]],
  ["removes x=x in middle", [xinc, new ast.Assignment(x, x), xinc], [xinc, xinc]],
  //null or undefined?
  ["optimizes if-true", new ast.IfStatement(true, xinc, null), xinc],
  ["optimizes if-false", new ast.IfStatement(false, xinc, null), xinc],
  ["optimizes while-false", [new ast.WhileStatement(false, xinc)], []],
//   ["optimizes repeat-0", [new ast.RepeatStatement(0, xinc)], []],
//   ["optimizes for-range", [new ast.ForRangeStatement(x, 5, "...", 3, xinc)], []],
//   ["optimizes for-empty-array", [new ast.ForStatement(x, emptyArray, xinc)], []],
  ["applies if-false after folding", new ast.ShortIfStatement(eq(1, 1), xinc), xinc],
